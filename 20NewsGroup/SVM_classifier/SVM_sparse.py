...
# c = 0
def add_sparse(word1,word2):
		
	# a,b = word1.shape
	t = time.time()
	i=0
	while i<60:
		if word1[i][0]==0 and word2[i][0]==0:
			i=i+1
			continue
		elif word1[i][0]!=0 and word2[i][0]!=0:
...
def create_cluster_vector_and_gwbowv_sparse(wtv,wordlist,final_dimension,train=False):
    # global c
    # This function computes SDV feature vectors.
    bag_of_centroids = np.zeros((num_clusters,embed_dim))
...
if reduction == 0 :
	name = "wtv"
	print("Using the non-reduced WTV file (reduced wtv)")
else:
	name = "reduced_wtv"
	print("Using the reduced WTV file (reduced wtv)")
...
for i in wtv :
    ...
    # print(type(wtv[i]))
    wtv[i] = wtv[i].reshape((num_clusters,embed_dim)) 
    # wtv[i] = sp(wtv[i])
    # print()
...
# for i in wtv:
# 	# print(wtv[i].shape)
# 	assert ( final_dimension == wtv[i].shape[1])
...
for review in train["news"]:
    ...
    s = time.time()
    t = create_cluster_vector_and_gwbowv_sparse(wtv,words,final_dimension,train=True)
    
    t = t.reshape((1,final_dimension))
    cnt += time.time()-s
    gwbowv[counter] = t
    counter+=1
    if counter % 1000 == 0:
...
for review in test["news"]:
    ...
    s = time.time()
    t = create_cluster_vector_and_gwbowv_sparse(wtv,words,final_dimension)

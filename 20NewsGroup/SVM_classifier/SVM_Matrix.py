...
for review in train["news"]:
    ...
    s = time.time()
    t = (create_cluster_vector_and_gwbowv(wtv,words,num_clusters,word2vec_dim,train=True)).flatter()
    cnt += time.time()-s
    gwbowv[counter] = t
    counter+=1
    if counter % 1000 == 0:
...
for review in test["news"]:
    ...
    s = time.time()
    t = (create_cluster_vector_and_gwbowv(wtv,words,num_clusters,word2vec_dim)).flatter()
    cnt += time.time()-s
